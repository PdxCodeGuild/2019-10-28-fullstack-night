...
let ball1 = {
    ...
    radius: 75,
    mass: 75,
    px: Math.random()*(width - 75),
    py: Math.random()*(height - 75),
...
function main_loop() {
    ...
    for (let i=0; i < ballsArray.length; i++) {
        ...
        if (ballsArray[i].px < ballsArray[i].radius || ballsArray[i].px > width - ballsArray[i].radius) {
            ballsArray[i].vx *= -1
            // ballsArray[i].vx *= .99 //friction
            // ballsArray[i].vy *= .99 //friction
        }
        if (ballsArray[i].py < ballsArray[i].radius || ballsArray[i].py > height - ballsArray[i].radius) {
            ballsArray[i].vy *= -1
            // ballsArray[i].vx *= .99 //friction
            // ballsArray[i].vy *= .99 //friction
        }
        ...
        if (ballA.px + ballA.radius + ballB.radius > ballB.px 
            ...
            && ballA.py < ballB.py + ballA.radius + ballB.radius) {
                ...
                distance = Math.sqrt(
                    ...
                    if (distance <= ballA.radius + ballB.radius && collisionBool === false) {   
                        collisionCount ++
